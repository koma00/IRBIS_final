...
namespace AM.Collections
{
          ...
          ICloneable
    {
        ...
        public BidirectionalDictionary
            (
            IEqualityComparer < TKey > straightComparer,
            IEqualityComparer < TValue > reverseComparer )
        {
        ...
        {
            return ( (IDictionary < TKey, TValue >) _straight ).Contains
                ( item );
        }
            ...
            int arrayIndex )
        {
            lock ( _syncRoot )
            {
                ( (IDictionary < TKey, TValue >) _straight ).CopyTo
                    (
                     array,
                     arrayIndex );
            }
        ...
        private bool _Remove ( ref KeyValuePair < TKey, TValue > item )
        {
            ...
            lock ( _syncRoot )
            {
                bool straightSucces = _straight.Remove ( item.Key );
                bool reverseSuccess = _reverse.Remove ( item.Value );
                Debug.Assert ( straightSucces == reverseSuccess );
                return straightSucces;
            }
        }
